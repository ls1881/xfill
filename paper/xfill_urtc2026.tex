...
\caption{Wall time to solve (log scale, 30s cap), all five solvers, per scraped $15\times15$ grid, sorted by xfill's time. Hatched bars mark a timeout; the naive baselines (crossword-composer, savin\_crossword) time out on most grids, the same pattern reported by count in Section~\ref{sec:naive}. Label above the xfill/orca-solver pair is orca-solver time / xfill time; only \texttt{grid\_479} favors orca-solver.}
\label{fig:speedup}
\end{figure*}
