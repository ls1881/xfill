...
\begin{table}[t]
\centering
\caption{Thread count vs.\ wall time on a known-hard $7\times7$ grid (single sample; published single-threaded partition-based baseline: 201.7s).}
\label{tab:threads}
\begin{tabular}{@{}lr@{}}
\toprule
Threads & Wall time (s) \\
...
Across all 20 grids, xfill solved 17 (1 correct UNSAT, 2 timeouts), orca-solver 16 (1 UNSAT, 3 timeouts), ingrid\_core 12, crossword-composer 5, and savin\_crossword \textit{0} -- it timed out on every grid, including the trivial $5\times5$, and so never appears in Fig.~\ref{fig:size}. That absence is itself informative: crossword-composer, which does appear intermittently, fails on individual instances despite having no dependence on dictionary size, while savin\_crossword's failure is not about grid size at all -- its least-constraining-value step recomputes an $O(|\text{domain}|^2)$ score from scratch at every node, which our $\sim$184{,}000-word dictionary makes prohibitive regardless of size. Both are real, unmodified, independently authored solvers. The point for this paper's thesis is narrower than ``xfill wins'': at the grid and dictionary sizes crossword filling actually requires, restarts, adaptive heuristics, and parallelism are not optional refinements -- they are the premise on which the rest of this section, and the mechanism this paper contributes, depend.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../benchmarks/urtc2026_testbench/results/figures/fig1b_speedup.pdf}
\caption{Per-grid speedup (orca-solver time / xfill time, log scale) on the scraped $15\times15$ grids both solvers solved. Bars above the line mean xfill is faster; only one of 10 falls below it.}
\label{fig:speedup}
\end{figure}

\subsection{Aggregate Speed Advantage Where Both Solve}
Fig.~\ref{fig:speedup} quantifies the restart-portfolio/partition comparison on the scraped $15\times15$ corpus alone \cite{crosswordgrids} -- realistic-scale grids, rather than the curated size-graded set, which mixes in small grids that inflate the ratio. Of the 10 scraped grids (of 12) both xfill and orca-solver solve, xfill is faster on 9, with a median speedup of $428\times$ and a geometric mean of $108\times$, driven by grids where xfill finishes in single-digit milliseconds against 3--13 seconds for orca-solver. The one exception, \texttt{grid\_479}, runs the other way (orca-solver $\sim\!3\times$ faster) -- a reminder that a lopsided aggregate is still not a universal one, consistent with \texttt{grid\_115} in Section~\ref{sec:scaling}. Across the full 20-grid sample, both solve 16, agree on one correct UNSAT proof, and are jointly stuck on 2, with orca-solver never uniquely solving a grid xfill cannot -- this sample is too easy to separate the two architectures on success alone, which is exactly why Sections~\ref{sec:scaling} and~\ref{sec:ablation} use a harder, pre-existing grid list to find where the architectures actually diverge.
...
\bibitem{crosswordgrids} Crosswordgrids.com, ``15x15 crossword grids.'' [Online]. Available: \url{https://crosswordgrids.com/15x15-crossword-grids/}. Accessed: Jul. 31, 2026.
