...
1. Shared-conflict-weight ablation: xfill run twice per grid, identical in
   every way except the XFILL_DISABLE_SHARED_WEIGHTS env var -- isolates
   this paper's actual mechanism from every other difference between
   solvers (dictionary, language, everything), which comparing against a
   different codebase (orca-solver) cannot do.
2. Thread-count scaling, xfill vs. orca-solver, on the same grids -- shows
   the two architectures respond to added parallelism in opposite ways.

Grid set: a fixed, already-curated "known hard" list reused from this
